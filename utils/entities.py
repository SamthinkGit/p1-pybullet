...
def aling_to_ground(object_id: int, x: float = 0.0, y: float = 0.0, orientation: list = [0,0,0,1], shift: float = 0.02) -> None:
    """
    Sets the position of an entity aligned to ground
    Assumes ground is the plane 0,0
    Args:
        param x: x location
        param y: y location
        param orientation: Orientation of the entity in quaternion
        param shift: Shift upward or downward the object after alineation
    """
    aabb = pb.getAABB(object_id)
    pb.resetBasePositionAndOrientation(object_id, [x, y, aabb[1][2] + shift], orientation)
    return None
